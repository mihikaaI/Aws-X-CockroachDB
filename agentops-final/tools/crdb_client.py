...
def run_query(sql, params=None, fetch=True, commit=None):
    ...
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            # Pass params through as-is rather than coercing None -> () --
            # psycopg2 treats *any* non-None value (even an empty tuple) as
            # "this query has %s placeholders to substitute", which then
            # misparses a literal `%` in the SQL itself (e.g. `ILIKE '%x%'`)
            # as a format specifier and raises a confusing
            # "IndexError: tuple index out of range". None disables
            # substitution entirely, which is what a truly paramless query
            # (including ones with literal % characters) needs.
