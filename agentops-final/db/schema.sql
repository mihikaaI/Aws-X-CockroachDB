...
CREATE TABLE IF NOT EXISTS orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    -- NOTE: intentionally NOT a `REFERENCES customers(id)` foreign key.
    -- CockroachDB auto-creates a secondary index on any FK-constrained
    -- column that doesn't already have one (FK columns must be indexed),
    -- which would silently defeat the "no index on customer_id" premise
    -- this demo depends on. Referential integrity here is enforced at the
    -- application/seed level instead.
