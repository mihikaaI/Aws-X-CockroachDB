...
``log`` takes an optional ``data`` mapping that is stored as JSONB, so the
live dashboard can render structured fields (latency, confidence, full_scan,
...) instead of parsing them back out of a human-readable sentence.
"""
import json
...
class Agent:
    ...
    def log(self, incident_id, step, detail="", data=None):
        console.print(f"[bold cyan]{self.name}[/bold cyan] · {step} — {detail}")
        try:
            crdb_client.run_query(
                """INSERT INTO agent_trace (incident_id, agent_name, step, detail, data)
                   VALUES (%s, %s, %s, %s, %s)""",
                (
                    incident_id,
                    self.name,
